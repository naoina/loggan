...
func TestLogger_Debug(t *testing.T) {
	ctime := time.Now()
	loggan.Now = func() time.Time { return ctime }
	defer func() { loggan.Now = time.Now }()
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
		{loggan.NONE, expected},
		{loggan.DEBUG, expected},
		{loggan.INFO, ""},
		{loggan.WARN, ""},
		{loggan.ERROR, ""},
		{loggan.FATAL, ""},
		...
		buf.Reset()
		logger := loggan.New(&buf, &loggan.LTSVFormatter{}, v.level)
		logger.Debug(msg)
		actual := buf.String()
		expected := v.expected
...
func TestLogger_Debugf(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Info(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Infof(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Warn(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Warnf(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Error(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Errorf(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Panic(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
		...
		{loggan.WARN, expected},
		{loggan.ERROR, expected},
		{loggan.FATAL, expected},
		{loggan.PANIC, expected},
	} {
		buf.Reset()
		logger := loggan.New(&buf, &loggan.LTSVFormatter{}, v.level)
		func() {
...
func TestLogger_Panicf(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Print(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	expected := "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Printf(t *testing.T) {
	...
	format := "test log: %v"
	msg := "this is test"
	expected := "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_Output(t *testing.T) {
	...
	var buf bytes.Buffer
	msg := "test log"
	for _, currentLevel := range []loggan.Level{
		loggan.NONE,
		loggan.DEBUG,
		loggan.INFO,
		loggan.WARN,
		...
		for _, v := range []struct {
			...
			{loggan.NONE, "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.DEBUG, "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.INFO, "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.WARN, "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.ERROR, "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.FATAL, "level:FATAL\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
			{loggan.PANIC, "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\n"},
		} {
			buf.Reset()
			logger := loggan.New(&buf, &loggan.LTSVFormatter{}, currentLevel)
...
func TestLogger_With_Debug(t *testing.T) {
	...
	expected := "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
		...
		buf.Reset()
		logger := loggan.New(&buf, &loggan.LTSVFormatter{}, v.level).With(fields)
		logger.Debug(msg)
		actual := buf.String()
		expected := v.expected
		if !reflect.DeepEqual(actual, expected) {
...
func TestLogger_With_Debugf(t *testing.T) {
	...
	expected := "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Info(t *testing.T) {
	...
	expected := "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Infof(t *testing.T) {
	...
	expected := "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Warn(t *testing.T) {
	...
	expected := "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Warnf(t *testing.T) {
	...
	expected := "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Error(t *testing.T) {
	...
	expected := "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Errorf(t *testing.T) {
	...
	expected := "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Panic(t *testing.T) {
	...
	expected := "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Panicf(t *testing.T) {
	...
	expected := "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Print(t *testing.T) {
	...
	expected := "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Printf(t *testing.T) {
	...
	expected := "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log: this is test\tfirst:1\tsecond:two\n"
	for _, v := range []struct {
		level    loggan.Level
		expected string
	}{
...
func TestLogger_With_Output(t *testing.T) {
	...
	for _, currentLevel := range []loggan.Level{
		...
		for _, v := range []struct {
			...
			{loggan.NONE, "level:NONE\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.DEBUG, "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.INFO, "level:INFO\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.WARN, "level:WARN\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.ERROR, "level:ERROR\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.FATAL, "level:FATAL\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
			{loggan.PANIC, "level:PANIC\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:test log\tfirst:1\tsecond:two\n"},
		} {
			buf.Reset()
			logger := loggan.New(&buf, &loggan.LTSVFormatter{}, currentLevel).With(fields)
			logger.Output(v.level, msg)
			actual := buf.String()
			expected := v.expected
			if !reflect.DeepEqual(actual, expected) {
...
func TestLogger_Level(t *testing.T) {
	for _, level := range []loggan.Level{
		loggan.NONE,
		loggan.DEBUG,
		loggan.INFO,
		loggan.WARN,
		...
		logger := loggan.New(ioutil.Discard, &loggan.LTSVFormatter{}, level)
		actual := logger.Level()
		expected := level
		if !reflect.DeepEqual(actual, expected) {
...
func TestLogger_SetLevel(t *testing.T) {
	...
	defer func() { loggan.Now = time.Now }()
	var buf bytes.Buffer
	logger := loggan.New(&buf, &loggan.LTSVFormatter{}, loggan.DEBUG)
	var actual interface{} = logger.Level()
	var expected interface{} = loggan.DEBUG
	if !reflect.DeepEqual(actual, expected) {
	...
	logger.Debug(msg)
	actual = buf.String()
	expected = "level:DEBUG\ttime:" + ctime.Format(time.RFC3339Nano) + "\tmessage:" + msg + "\n"
	if !reflect.DeepEqual(actual, expected) {
		t.Errorf(`logger.Debug(%#v) prints %#v; want %#v`, msg, actual, expected)
	}
	logger.SetLevel(loggan.INFO)
	actual = logger.Level()
	expected = loggan.INFO
	if !reflect.DeepEqual(actual, expected) {
		t.Errorf(`After logger.SetLevel(%v); logger.Level() => %v; want %v`, loggan.INFO, actual, expected)
	}
	buf.Reset()
	logger.Debug(msg)
	actual = buf.String()
	expected = ""
	if !reflect.DeepEqual(actual, expected) {
		t.Errorf(`After logger.SetLevel(%v); logger.Debug(%#v) => %#v; want %#v`, loggan.INFO, msg, actual, expected)
	}
}
...
func TestLevel_String(t *testing.T) {
	for _, v := range []struct {
		...
		{loggan.NONE, "NONE", "NONE"},
		{loggan.DEBUG, "DEBUG", "DEBUG"},
		{loggan.INFO, "INFO", "INFO"},
		{loggan.WARN, "WARN", "WARN"},
		{loggan.ERROR, "ERROR", "ERROR"},
		{loggan.FATAL, "FATAL", "FATAL"},
		{loggan.PANIC, "PANIC", "PANIC"},
	} {
